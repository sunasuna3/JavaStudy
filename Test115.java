...
※ 인터페이스의 멤버 변수인 데이터는
   접근제어지시자를 명시하지 않아도 기본 상수인 
   static final 의 변경자로 설정된다.
   클래스에서 인터페이스를 추가하여 사용할 경우
   인터페이스 안에서 정의된 모든 메소들을 구현해 주어야 하며
   인터페이스를 구현하는 클래스는
   인터페이스의 상위 인터페이스가 제공하는추상 메소드를 포함한
   모든 메소드를 구현하지 않을 경우
...
※ 인터페이스 구현
	...
	이러한 기능을 하는 것이 implements 예악어이다.

※ 클래스는 동시에 두개 이상의 인터페이스를 
   implements 할 수 있다~!!!
